...
var directClient = new PipeClient();
directClient.MessageReceived += ReceivedMessageHandler;
directClient.Connect(MODULE_NAME, "VideoStreamerPipe");
...
void ReceivedMessageHandler(object? eventSender, PipeMessageEventArgs message)
{
    if (message.TartaMessage.Type == "message" && message.TartaMessage.SubCategory == "videoframe")
    {
        var encodedFrame = Convert.FromBase64String(message.TartaMessage.Payload);
